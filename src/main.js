...
const renderer = new THREE.WebGLRenderer({ canvas: document.querySelector('#app'), alpha:true, antialias:true });
...
void main(){
  ...
  vec2 h=vec2(0.5); float sdf=roundSDF(vUv-h,h-u_radius,u_radius);
  float a=1.0-smoothstep(0.0,0.005,sdf);
...
for(let i=0;i<150;i++){const m=new THREE.MeshBasicMaterial({map:twinkleTex,color:0xffffff,transparent:true,opacity:Math.random()*0.5,blending:THREE.AdditiveBlending,depthWrite:false});
  ...
  mesh.isTwinkling=false; twinkleGrp.add(mesh);
}
let shooting=false;
...
const makeText=(str,size,col=0xffffff)=>{
  const t=new Text(); t.text=str; t.fontSize=size; t.color=col; t.anchorX='center';
  t.font='https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_2108@1.1/SpoqaHanSansNeo-Regular.woff';
  t.material.transparent=true; t.material.opacity=0; t.sync(); scene.add(t); return t;
};
...
pSlogan.anchorX='left'; pSlogan.lineHeight=1.5;
...
const projects=[
  {title:'AURA: Generative Data Sculpture',role:'Lead Developer & Creative Technologist',desc:'소셜 미디어의 실시간 텍스트 데이터를 감정적으로 분석하고...'
  ,tech:'Three.js, GSAP, Socket.IO, Python, AWS',img:projectAuraImg},
  {title:'Project: ODYSSEY',role:'Lead 3D Developer & Narrative Designer',desc:'소행성 ‘2025-ODYSSEY’ 탐사 로버의 로그를 따라가는 3D 웹 스토리...'
  ,tech:'Three.js, Blender, GSAP, GLSL',img:projectOdysseyImg},
  {title:'FlowField: Real-time Audio-Reactive Art',role:'Sole Developer & Artist',desc:'Web Audio API로 실시간 사운드를 분석해 유기적 비주얼을 만드는 미디어 아트...'
  ,tech:'TypeScript, p5.js, Web Audio API, GLSL, Vite',img:projectFlowfieldImg},
];
projects.forEach((p,i)=>{
  const g=new THREE.Group(); g.visible=false; g.position.z=-100*(i+1);
  const img=new THREE.Mesh(new THREE.PlaneGeometry(16,9),mkShaderMat(p.img)); img.position.set(-18,1,0);
  g.userData.shader=[img.material.uniforms.u_opacity];
  const tTitle=makeText(p.title,2); tTitle.anchorX='left'; tTitle.position.set(0,6.5,0);
  const tRole =makeText(p.role,1,0x00bfff); tRole.anchorX='left'; tRole.position.set(0,4,0);
  const tDesc =makeText(p.desc,1,0xcccccc); tDesc.anchorX='left'; tDesc.maxWidth=20; tDesc.lineHeight=1.5; tDesc.position.set(0,.5,0);
  const tLab  =makeText('Tech:',1); tLab.anchorX='left';
  const tTech =makeText(p.tech,.9,0xcccccc); tTech.anchorX='left'; tTech.maxWidth=20; tTech.lineHeight=1.5;
  ...
  tDesc.sync(()=>{const h=tDesc.geometry.boundingBox.max.y - tDesc.geometry.boundingBox.min.y;
    tLab.position.set(0,tDesc.position.y-h-1.5,0); tTech.position.set(0,tLab.position.y-1.2,0);});
...
tl.to([camera.position,spark.position],{z:o=>o===camera.position?10:0,duration:8,ease:'power2.inOut'},'star');
tl.to(spark.material,{size:3,duration:8,ease:'power2.inOut'},'star');
tl.to(spark.material,{opacity:0,duration:1.5,ease:'power2.out'},'>');
tl.set(spark,{visible:false});

/* Act2 – HUD in */
coordTxt.position.set(-28,0,0);
idTxt   .position.set(   0,0,0);
coreTxt .position.set(  28,0,0);
tl.addLabel('hud');
...
vLine.position.set(-7,0,0); profilePlane.position.set(-15,0,0);
pTitle .position.set(-1,4,0); pSlogan.position.set(-1,0.5,0);
emailL .position.set(-1,-3,0); emailV .position.set(2.5,-3,0);
gitL   .position.set(-1,-4.5,0); gitV   .position.set(3.2,-4.5,0);
/* ★ 카메라 X 이동 → plane (-15) 쪽으로 맞춰줌 */
